...
class _TypeAheadFieldState<T> extends State<TypeAheadField<T>> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _suggestionsBox = _SuggestionsBox(context, widget.direction, widget.autoFlipDirection);
    ...
    _keyboardVisibilitySubscription = _keyboardVisibility?.listen((bool isVisible) {
...
class _SuggestionsListState<T> extends State<_SuggestionsList<T>> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) {
    bool isEmpty = (_suggestions?.length ?? 0) == 0 && widget.controller!.text == "";
    if ((_suggestions == null || isEmpty) && _isLoading == false && _error == null) return Container();
    ...
    final animationChild = widget.transitionBuilder != null
        ...
        : SizeTransition(
            axisAlignment: -1.0,
            sizeFactor: CurvedAnimation(parent: _animationController!, curve: Curves.fastOutSlowIn),
            child: child,
          );
...
class _SuggestionsBox {
  ...
  _SuggestionsBox(this.context, this.direction, this.autoFlipDirection) : desiredDirection = direction;
  ...
  Future<bool> _waitChangeMetrics() async {
    if (widgetMounted) {
      ...
      EdgeInsets initial = MediaQueryData.fromWindow(WidgetsBinding.instance.window).viewInsets;
      ...
      while (widgetMounted && timer < waitMetricsTimeoutMillis) {
        ...
        if (widgetMounted &&
            (MediaQueryData.fromWindow(WidgetsBinding.instance.window).viewInsets != initial || _findRootMediaQuery() != initialRootMediaQuery)) {
          return true;
        }
      }
    }
  ...
  void _adjustMaxHeightAndOrientation() {
    ...
    double keyboardHeight = EdgeInsets.fromWindowPadding(WidgetsBinding.instance.window.viewInsets, devicePixelRatio).bottom;
  ...
  double _calculateMaxHeight(AxisDirection direction, RenderBox box, TypeAheadField widget, double windowHeight, MediaQuery rootMediaQuery,
      double keyboardHeight, double textBoxAbsY) {
  ...
  double _calculateMaxHeightDown(
      RenderBox box, TypeAheadField widget, double windowHeight, MediaQuery rootMediaQuery, double keyboardHeight, double textBoxAbsY) {
  ...
  double _calculateMaxHeightUp(
    ...
    directionUpOffset =
        textBoxAbsY > keyboardAbsY ? keyboardAbsY - textBoxAbsY - widget.suggestionsBoxVerticalOffset : -widget.suggestionsBoxVerticalOffset;
